...
def manage_customers():
    ...
    return render_template('customer/customers.html', customers=customers, percent=percent, search=search)

@customer_card_bp.route('/edit/<card_id>', methods=['GET', 'POST'])
@login_required
def edit_customer(card_id):
    conn = get_db_connection()
    cust = conn.execute('SELECT * FROM Customer_Card WHERE card_number = ?', (card_id,)).fetchone()
    ...
    if request.method == 'POST':
        ...
        percent_str = request.form.get('percent', '0')

        # Валідація
        error = None
        if len(phone) > 13:
            error = "Номер телефону не може перевищувати 13 символів!"
        ...
        try:
            ...
            error = "Відсоток має бути цілим числом від 0 до 100!"
        ...
        try:
            conn.execute('''
                ...
                WHERE card_number=?
            ''', (surname, name, patronymic, phone, city, street, zip_code, percent_val, card_id))
            conn.commit()
